...
func SaveFood(food models.Food, upc string) {
	...
	if err != nil {
		...
		err := row.Scan(&foodPlaceHolder.PantryID, &foodPlaceHolder.FoodID, &foodPlaceHolder.Barcode, &foodPlaceHolder.Title, &foodPlaceHolder.Nutriments.Calories,
			&foodPlaceHolder.Nutriments.Fat, &foodPlaceHolder.Nutriments.Carbohydrate, &foodPlaceHolder.Nutriments.Protein, pq.Array(&foodPlaceHolder.Misc))
		fmt.Println(err)
		if err != nil {
			rows, err := db.Query("INSERT INTO food(pantry_id, barcode, title, calories, fat, carbohydrate, protein, misc) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)",
				food.PantryID, upc, food.Title, food.Nutriments.Calories, food.Nutriments.Fat, food.Nutriments.Carbohydrate, food.Nutriments.Protein, pq.Array(food.Misc))
			if err != nil {
				log.Fatal(err)
			}
			defer rows.Close()
